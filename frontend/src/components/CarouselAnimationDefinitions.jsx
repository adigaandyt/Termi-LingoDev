...
const Card = ({title,concept,alertShow,alertToggleShow,isFavorite,setIsFavorite}) => {
  if(title===1){
    return(
    <div className='bg-light card-carousel2'>
      {/* hebrew*/}             
      <Definitions setIsFavorite={setIsFavorite} isFavorite={isFavorite}  alertShow={alertShow} alertToggleShow={alertToggleShow} languageChoosed={{english:false,hebrew:true,arabic:false,}} concept={concept}/>
    </div>
    )
  }else if(title===2){
    return(
      <div className='bg-light card-carousel2'>
      {/* arabic */}             
      <Definitions setIsFavorite={setIsFavorite} isFavorite={isFavorite}  alertShow={alertShow} alertToggleShow={alertToggleShow} languageChoosed={{english:false,hebrew:false,arabic:true,}} concept={concept}/>
      </div> 
    )
  }else if(title===3){
    return(
    <div className='bg-light card-carousel2'>
      {/* english */}        
      <Definitions setIsFavorite={setIsFavorite} isFavorite={isFavorite}  alertShow={alertShow} alertToggleShow={alertToggleShow} languageChoosed={{english:true,hebrew:false,arabic:false,}} concept={concept}/>
    </div>
  )}
};
...
const CarouselAnimationDefinitions= ({alertShow,alertToggleShow,concept}) => {
  ...
  const {conceptIsFavorite ,isSingleConceptSuccess}= useSelector(state=>state.concept)
    const [isFavorite, setIsFavorite] = useState(conceptIsFavorite);
    useEffect(()=>{
      if(concept)
      setIsFavorite(conceptIsFavorite)
    },[conceptIsFavorite,isSingleConceptSuccess])
  
    return(
    <div   id='body'>
         

      <Carousel  concept={concept} isFavorite={isFavorite} setIsFavorite={setIsFavorite} >
        {[...new Array(CARDS)].map((_, i) => (
          <Card alertShow={alertShow} alertToggleShow={alertToggleShow} concept={concept} setIsFavorite={setIsFavorite} isFavorite={isFavorite}  title={ (i + 1)} content='Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.'/>
        ))}
