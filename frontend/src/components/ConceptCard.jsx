    ...
    return(<>
        <div id='parent '>
            <div class="card my-2 w-100" id='child'>
                <div class="card-body">
                    <h6 class="card-title">{getConceptName(languageChoosed,concept)}</h6>
                    <p class="card-subtitle mb-2 text-secondary">{searchById(concept.categories[0])}</p>
                    {concept.accepted?
                        <p class="card-subtitle mb-2 text-success">{t('accepted_by_admin')} </p>:
                        <p class="card-subtitle mb-2 text-danger">{t('not_accepted_by_admin_yet')}</p>}
